
import React, { useEffect, useState } from 'react'
import './proffesionalProfile.css';
import { useParams } from 'react-router';
import { API_URL, MY_INFO, MY_PROINFO, doApiMethod } from '../services/apiService';
import { MDBCol, MDBContainer, MDBRow, MDBCard, MDBCardText, MDBCardBody, MDBCardImage, MDBTypography, MDBIcon } from 'mdb-react-ui-kit';
import UserProfile from '../comps_users/userProfile';
import UserProfileInfo from '../comps_users/userProfileInfo';
import NavbarGeneralProffesionel from '../comps_general/navbarGeneralProffesionel';
import { Link } from 'react-router-dom';

export default function ProffesionalProfile() {
    const [info, setInfo] = useState({});
    const params = useParams();

    useEffect(() => {
        doApiInit();
    }, [])


    const doApiInit = async () => {
        if (localStorage[MY_PROINFO]) {
            setInfo(JSON.parse(localStorage[MY_PROINFO]));
        } else {
            let url = API_URL + "/proffesionals/myInfo";
            try {
                let resp = await doApiMethod(url, "GET");
                console.log(resp.data);
                setInfo(JSON.parse(localStorage[MY_PROINFO]));
            }
            catch (err) {
                console.log(err);
                alert("There problem try come back later");
            }
        }

    }

    return (
        <section className="vh-100" style={{ backgroundColor: 'RGB(235 250 255)' }}>
            <NavbarGeneralProffesionel />
            <MDBContainer className="py-5 h-100">
                <MDBRow className="justify-content-center align-items-center h-100">
                    <MDBCol lg="6" className="mb-4 mb-lg-0">
                        <MDBCard className="mb-3" style={{ borderRadius: '.5rem' }}>
                            <MDBRow className="g-0">
                                <MDBCol md="4" className="text-center"
                                    style={{ borderTopLeftRadius: '.5rem', borderBottomLeftRadius: '.5rem', background: "rgb(250, 244, 238)" }}>
                                    {info.img_url ? <img src={info.img_url}
                                        alt="name" className="my-5" style={{ width: '100px', borderRadius: '50%', border: "solid 2px rgb(235 188 127) " }} fluid /> : <img src="data:image/jpeg;base64,"
                                            alt="name" className="my-5" style={{ width: '100px', borderRadius: '50%', border: "solid 2px rgb(235 188 127) " }} fluid />}
                                    <MDBTypography style={{ color: "rgb(235 188 127)" }} tag="h5">{info.name?.firstName} {info.name?.lastName}</MDBTypography>
                                    <MDBCardText style={{ color: "rgb(235 188 127)" }}>{info.category}</MDBCardText>
                                    <Link to={`/proffesionals/editProffesional/${info._id}`}><MDBIcon far icon="edit mb-5" style={{ color: "rgb(235 188 127)" }} /></Link>
                                </MDBCol>
                                <MDBCol md="8">
                                    <MDBCardBody className="p-4">
                                        <MDBTypography tag="h6" className='display-6'>My profile

                                        </MDBTypography>
                                        <hr className="mt-0 mb-4" />
                                        <UserProfileInfo info={info} />
                                        <br />
                                        <MDBRow className="pt-1">
                                            <MDBCol size="3" className="mb-3">
                                                <MDBTypography tag="h6">Area</MDBTypography>
                                                <MDBCardText className="text-muted">{info.area}</MDBCardText>
                                            </MDBCol>
                                            <MDBCol size="3" className="mb-3">
                                                <MDBTypography tag="h6">Cost</MDBTypography>
                                                <MDBCardText className="text-muted">{info.cost}</MDBCardText>
                                            </MDBCol>
                                            <MDBCol size="5" className="mb-3">
                                                <MDBTypography tag="h6">Event types</MDBTypography>
                                                {/* <MDBCardText className="text-muted">{info.event_type}</MDBCardText> */}
                                                <div >
                                                    {info.event_type?.length > 0 &&
                                                        info.event_type?.map((item, index) => (

                                                            <div >
                                                                {item},
                                                            </div>

                                                        ))}
                                                </div>
                                            </MDBCol>
                                        </MDBRow>
                                        {info.ig_url ? <div className="d-flex justify-content-start">
                                            {/* <a href="#!"><MDBIcon fab icon="facebook me-3" size="lg" /></a>
                                            <a href="#!"><MDBIcon fab icon="twitter me-3" size="lg" /></a> */}
                                            <a style={{ color: 'rgb(134, 80, 80)' }} href={info.ig_url} target='blank'><MDBIcon fab icon="instagram me-3" size="lg" /> My Instagram</a>
                                        </div> : ""}

                                    </MDBCardBody>
                                </MDBCol>
                            </MDBRow>
                        </MDBCard>
                    </MDBCol>
                </MDBRow>
            </MDBContainer>
        </section>
    );
}

